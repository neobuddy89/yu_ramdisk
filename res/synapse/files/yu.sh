...
case "$1" in
	...
	;;
	DefaultGPUGovernor)
		$BB echo "`$BB cat /sys/devices/soc.0/1c00000.qcom,kgsl-3d0/devfreq/1c00000.qcom,kgsl-3d0/governor`"
	;;
	DirKernelIMG)
	...
	;;
	DirGPUGovernor)
		$BB echo "/sys/devices/soc.0/1c00000.qcom,kgsl-3d0/devfreq/1c00000.qcom,kgsl-3d0/governor";
	;;
	DirGPUMaxFrequency)
		$BB echo "/sys/devices/soc.0/1c00000.qcom,kgsl-3d0/devfreq/1c00000.qcom,kgsl-3d0/max_freq";
	;;
	DirGPUMinPwrLevel)
		$BB echo "/sys/devices/soc.0/1c00000.qcom,kgsl-3d0/devfreq/1c00000.qcom,kgsl-3d0/min_freq";
	;;
	#DirGPUNumPwrLevels)
	...
	;;
	GPUFrequencyList)
		for GPUFREQ in `$BB cat /sys/devices/soc.0/1c00000.qcom,kgsl-3d0/devfreq/1c00000.qcom,kgsl-3d0/available_frequencies` ; do
		LABEL=$((GPUFREQ / 1000000));
			$BB echo "$GPUFREQ:\"${LABEL} MHz\", ";
		done;
	;;
	GPUGovernorList)
		for GPUGOV in `$BB cat /sys/devices/soc.0/1c00000.qcom,kgsl-3d0/devfreq/1c00000.qcom,kgsl-3d0/available_governors`; do
			$BB echo "\"$GPUGOV\",";
		done;
	;;
	GPUPowerLevel)
	...
	;;
	LiveCPUTemperature)
		CPU_C=`$BB awk '{ print $1 }' /sys/class/thermal/thermal_zone5/temp`;
		CPU_F=`$BB awk "BEGIN { print ( ($CPU_C * 1.8) + 32 ) }"`;
		CPU_C1=`$BB awk '{ print $1 }' /sys/class/thermal/thermal_zone4/temp`;
		CPU_F1=`$BB awk "BEGIN { print ( ($CPU_C1 * 1.8) + 32 ) }"`;

		$BB echo "CPU0-1: $CPU_C°C | $CPU_F°F@nCPU2-3: $CPU_C1°C | $CPU_F1°F";
	;;
	LiveGPUFrequency)
		GPUFREQ="$((`$BB cat /sys/devices/soc.0/1c00000.qcom,kgsl-3d0/kgsl/kgsl-3d0/gpuclk` / 1000000)) MHz";
		$BB echo "$GPUFREQ";
	;;
	LiveMemory)
